...
class SignatureDecoder
{
     ...
     * @return string
     */
    public function decode($signature, $js_code): string
    {
    ...
    /**
     * Parses from function name
     *
     * @param $js_code
     * @return string|null
     */
    public function parseFunctionName($js_code): ?string
    {
        if (preg_match('@,\s*encodeURIComponent\((\w{2})@is', $js_code, $matches)) {
            $func_name = $matches[1];
            return preg_quote($func_name);
     ...
     * @return array|null
     */
    public function parseFunctionCode($func_name, $player_html): ?array
    {
        ...
        if (preg_match('/' . $func_name . '=function\([a-z]+\){(.*?)}/', $player_html, $matches)) {
            ...
            if (preg_match_all('/([a-z0-9$]{2})\.([a-z0-9]{2})\([^,]+,(\d+)\)/i', $js_code, $matches)) {
                ...
                foreach ($matches2 as $m) {
                    ...
                    if (str_contains($m[2], 'splice')) {
                        $functions[$m[1]] = 'splice';
                    } elseif (str_contains($m[2], 'a.length')) {
                        $functions[$m[1]] = 'swap';
                    } elseif (str_contains($m[2], 'reverse')) {
